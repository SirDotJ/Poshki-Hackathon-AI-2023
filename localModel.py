...
VIDEO_PATH = "./data/input/11_20_42.mp4"
    ...
    if frameCounter % 6 == 0:
        ...
        for pedestrian in pedestrians:
            if pedestrian.intersects(dangerZone):
                if (abs(time.time() - lastDangerTimestamp)) >= DANGER_COOLDOWN:
                    print("DANGER")
...
capture.release()
# row = [VIDEO_PATH, eventCounter]
# if eventCounter == 0:
#     row.append('[]')
# else:
#     node = '['
#     node += eventTimestamps[0]
#     for i in range(len(eventTimestamps)):
#         node += ", " + eventTimestamps[i]
#     node = ']'
#     row.append(node)
